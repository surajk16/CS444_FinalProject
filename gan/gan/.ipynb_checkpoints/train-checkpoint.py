...
def train(D, G, D_solver, G_solver, discriminator_loss, generator_loss, model_save_path, show_every=250, 
    ...
    for epoch in range(num_epochs):
        ...
        for x, _ in train_loader:
            ...
            gen_output = G(noise).reshape(batch_size, input_channels, img_size, img_size)
            fake_img_scores = D(gen_output)
            real_img_scores = D(real_images)
            d_error = discriminator_loss(real_img_scores, fake_img_scores)
            d_error.backward()
            D_solver.step()
            ...
            gen_scores = D(fake_images)
            g_error = generator_loss(gen_scores)
            g_error.backward()
            G_solver.step()
            ...
            if (iter_count % show_every == 0):
                ...
                ## Save model
                torch.save(G.state_dict(), model_save_path + "generator/iter_{}.pth".format(iter_count))
                torch.save(D.state_dict(), model_save_path + "discriminator/iter_{}.pth".format(iter_count))
